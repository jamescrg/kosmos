"""Tests for the date-dropdown / Filter-button indicator coherence on Expenses.
...
def test_default_state_filter_button_off(client):
    response = client.get(reverse("activity:expenses-list"))
...
def test_last_week_dropdown_does_not_light_filter_button(client):
    client.post(reverse("activity:expenses-filter-quick", args=["last_week"]))
    response = client.get(reverse("activity:expenses-list"))
    assert response.context["custom_filter_active"] in (False, None, {})
...
def test_unbilled_preset_does_not_light_filter_button(client):
    client.post(reverse("activity:expenses-filter-quick", args=["unbilled"]))
    response = client.get(reverse("activity:expenses-list"))
    assert response.context["custom_filter_active"] in (False, None, {})
...
def test_modal_keyword_lights_filter_button(client):
    response = client.post(
        reverse("activity:expenses-filter"),
        {"description": "courier"},
    )
