...
def test_payments_list(client, payment):
    response = client.get(reverse("invoicing:payments-list"))
    assert response.status_code == 200

    assertTemplateUsed(response, "invoicing/payments/list.html")
...
def test_payments_add_get(client):
    response = client.get(reverse("invoicing:payments-add"))
    assert response.status_code == 200

    assertTemplateUsed(response, "invoicing/payments/form.html")
...
def test_payments_add_post(client, matter):
    ...
    response = client.post(reverse("invoicing:payments-add"), payment_data)
...
def test_payments_delete(client, payment):
    response = client.post(
        reverse("invoicing:payments-delete", kwargs={"pk": payment.pk})
    )
...
def test_payments_edit_get(client, payment):
    response = client.get(reverse("invoicing:payments-edit", kwargs={"pk": payment.pk}))

    assert response.status_code == 200
    assertTemplateUsed(response, "invoicing/payments/edit.html")
...
def test_payments_edit_post(client, payment):
    ...
    response = client.post(
        reverse("invoicing:payments-edit", kwargs={"pk": payment.pk}), updated_data
    )
    assert response.status_code == 204
...
def test_payments_filter_get(client):
    response = client.get(reverse("invoicing:payments-filter"))
    assert response.status_code == 200

    assertTemplateUsed(response, "invoicing/payments/filter.html")
    assert "filter" in response.context


def test_payments_filter_post(client):
    filter_data = {"payment_method": "BANK_TRANSFER"}
    response = client.post(reverse("invoicing:payments-filter"), filter_data)
